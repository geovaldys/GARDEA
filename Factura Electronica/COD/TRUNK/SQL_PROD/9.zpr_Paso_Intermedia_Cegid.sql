...
BEGIN
   ...
   ****************************************/
   ...
   IF OBJECT_ID('tempdb..#T_PIECE') IS NOT NULL
	...
	SELECT 
		...
		(SELECT TOP 1 MP_RACINEREFINT1 FROM [2CAP].[dbo].[MODEPAIE] WHERE MP_MODEPAIE=CMODEPAIE.MODEPAIE) GP_MODEPAIE_CODE
	FROM [2CAP].[dbo].[PIECE]
	OUTER APPLY
	(
		SELECT TOP 1 GPE_MODEPAIE MODEPAIE  FROM [2CAP].[dbo].[PIEDECHE]
		WHERE GPE_NUMERO=GP_NUMERO AND GPE_SOUCHE=GP_SOUCHE AND GPE_NATUREPIECEG=GP_NATUREPIECEG
		ORDER BY GPE_MONTANTECHE DESC
	)CMODEPAIE
	OUTER APPLY 
	(
		SELECT SUM(MLR_MONTANTHT) MONTANTTTC FROM [2CAP].[dbo].[LIGNEREMISE]
		WHERE MLR_NATUREPIECEG=GP_NATUREPIECEG AND MLR_NUMERO=GP_NUMERO AND MLR_SOUCHE=GP_SOUCHE
	) CREMISE
	OUTER APPLY
	(
		SELECT (SUM(GL_TOTALTTC)-SUM(GL_TOTALHT)) TOTALTAXEDEV1, SUM(GL_PUHT*ABS(GL_QTEFACT)) SUBTOTAL FROM [2CAP].[dbo].[LIGNE]
		WHERE GL_NUMERO=GP_NUMERO AND GL_SOUCHE=GP_SOUCHE AND GP_NATUREPIECEG=GL_NATUREPIECEG
	) CLIGNE
	OUTER APPLY
	(
	    SELECT  COUNT(*) CONTADOR FROM [2CAP].[dbo].[LIGNE]
		WHERE GL_NUMERO=GP_NUMERO AND GL_SOUCHE=GP_SOUCHE AND GP_NATUREPIECEG=GL_NATUREPIECEG
   ...
   ****************************************/
   ...
   INSERT INTO #T_LIGNE
	...
	FROM [2CAP].[dbo].[LIGNE]
	INNER JOIN [2CAP].[dbo].[PIECE] ON GP_NUMERO=GL_NUMERO AND GP_SOUCHE=GL_SOUCHE AND GP_NATUREPIECEG=GL_NATUREPIECEG
	INNER JOIN [2CAP].[dbo].[ETABLISS] ON ET_ETABLISSEMENT=GP_SOUCHE
	OUTER APPLY 
	(
		SELECT SUM(MLR_MONTANTHT) MONTANTTTC FROM [2CAP].[dbo].[LIGNEREMISE]
		WHERE MLR_NATUREPIECEG=GL_NATUREPIECEG AND MLR_NUMERO=GL_NUMERO AND MLR_SOUCHE=GL_SOUCHE AND MLR_NUMLIGNE=GL_NUMLIGNE
	...
	OUTER APPLY 
	(
		SELECT GXT_TAUXVTE FROM [2CAP].[dbo].[TAUXTAXE] 
